...
Start and EndThe start and the end points of the *incoming*  branch.ChildrenPointer to the first child or null if it is a leaf.BrotherPointer to the next sibling or null.FatherPointer to the node's father.Suffix LinkA node's suffix link.Note that a *father*  link is **not strictly necessary** . This means that it possible to have (and even build) a tree without *father links* .
...
StartA position in the original string where the string to the node (the full string from the root) appears. Note that there may be several positions in the string which fulfil this criterion. For representation purposes it does not matter which one is taken.String DepthHow deep is the node, measured by the number of characters that must be followed from the root to reach it. Note that this is different fromnode depthwhich is the number of intermediate nodes that are passed *en route*  to the node.
...
Here you can find my implementation of suffix trees in C++. I have implemented both McCreight's and Weiner's algorithms. Ukkonnen's algorithm can be shown to be just a variation of McCreight's (see `Kurtz's paper <#kurtz>`_ ), so you can consider that that one is implemented as well.

This code is used as the basis of my `dotted tree <dotted-tree>`_ software, but independent of it.
